...
Follow [providers/fly.md](providers/fly.md) to create separate controller and
worker apps, separate private networks, and the controller's durable volume.
Do not deploy the controller yet.
...
Create an app-scoped deployment token for the worker provider. It must be able
to create, list, and delete workers, but should have no access to production
applications.
...
Deploy the bundled Fly adapter using the generated controller configuration.
Other providers implement the [adapter contract](adapter-contract.md).
