...
| I2 | Done in part (2026-09-05): a fleet can run a [derived worker image](derived-images.md) declared with `RUNNER_IMAGE_BASE`, which is the operator-side path to a `full` tier today. Remaining, the named tiers:  `minimal` (current: runner, Node 22, Docker, git, gh, python3, build-essential) and `full` built from the upstream `actions/runner-images` install scripts, with a lock file pinning the upstream revision, a scheduled rebuild, and a provenance manifest. `RUNNER_IMAGE` selects the tier. | P2 | L |
| I3 | Partly covered by derived images (a fleet prewarms exactly what its workflows need; the example in the guide warms pnpm, Bun, ffmpeg, and Playwright). Remaining in the release image:  Node 20/22/24 LTS majors in the toolcache (a floating `node-version` currently downloads a newer patch), Python 3.12 and 3.13 for `actions/setup-python`, pnpm, and an optional Playwright browsers layer. | P1 | S |
