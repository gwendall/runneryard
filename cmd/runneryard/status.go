...
func writeFleetStatus(output io.Writer, status controller.FleetStatus, asJSON bool) error {
	...
	fmt.Fprintf(output, "Workers     %d actual  %d starting  %d busy  %d idle  %d orphan candidate(s)  max %d%s\n",
		status.Workers.Actual, status.Workers.Starting, status.Workers.Busy, status.Workers.Idle, status.Workers.OrphanCandidates, status.Workers.Maximum, capacity)
