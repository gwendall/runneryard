  ...
  FLY_API_TOKEN='<worker-app deploy token>'`}</CodeBlock>
          ...
          <section className="provider-section">
            ...
            <div className="provider-content">
              ...
              <CodeBlock>{`fly machine run ghcr.io/gwendall/runneryard:0.3.7 \\
...
fly deploy --app acme-ci-controller \\
  ...
  --image ghcr.io/gwendall/runneryard:0.3.7 --ha=false`}</CodeBlock>
              <p>Performance workers cost more per second than shared workers, but avoid the post-burst slowdown that can turn tests into timeouts and reruns. Measure cost per successful job and keep the operator-owned concurrency ceiling low until a representative canary proves the workload. <a href={`${docsUrl}/configuration.md#fly-worker-shape`}>Review worker sizing</a>.</p>
